...
<div class="content">
    <button type="button" class="btn btn-sm btn-dark mb-4" data-bs-toggle="modal" data-bs-target="#modal-add-categories"">
        <i class="fa fa-fw fa-plus me-1"></i> Add New Category
    </button>
                ...
                <table id="galleryTable"  class="table table-bordered table-striped table-vcenter js-dataTable-full-pagination">
                    <thead>
                        <tr>
                            ...
                            <th class="fs-xs" style="width: 65%">Description</th>
                            <th class="fs-xs text-center" style="width: 10%">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($category as $index => $kategori)
                        <tr>
                            <td class="text-center fs-sm">{{ ($index+1) }}</td>
                            <td class="fw-semibold fs-sm">{{ $kategori->name }}</td>
                            <td class="d-none d-sm-table-cell fs-sm">
                                {{ $kategori->description }}<span class="text-muted"></span>
                            </td>
                            <td class="fs-sm text-center">
                                <div class="btn-group">
                                    <button type="button" class="btn btn-sm btn-primary" onclick="editDataCategory('{{$kategori->id}}')">
                                        <i class="fa fa-fw fa-pencil-alt"></i>
                                    </button>
                                    <a type="button" data-id="{{ $kategori->id }}" class="btn btn-sm btn-danger btnDelete">
        ...
        <div class="modal-content">
            <div class="block block-rounded block-themed block-transparent mb-0">
                ...
                <div class="block-content fs-sm">
                    <form action="{{ url('admin/categories') }}" method="post" id="form-add-categories" enctype="multipart/form-data" >
                        @csrf
                        <div class="mb-3">
            ...
            <div class="block block-rounded block-themed block-transparent mb-0">
                ...
                <div class="block-content fs-sm">
                    <form action="" method="post" id="form-edit-category" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Category Name</label>
                            <input type="text" class="form-control" id="name_category" name="name" value="" required  />
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Category Description</label>
                            <textarea type="text" class="form-control" id="description_category" name="description" rows="4" required></textarea>
                        </div>
                    </form>
                </div>
                <div class="block-content block-content-full text-end bg-body">
...
<script type="text/javascript">
    ...
    $('#galleryTable').on("click", ".btnDelete", function() {
            ...
            if (result.value) {
                let url = "{{ url('admin/categories/delete/:id') }}";
    ...
    function editDataCategory(id){
        $.ajax({
            ...
            success: function(data){
                ...
                $('#modal-edit-category form').attr('action', "{{ url('/admin/categories/edit/update/') }}/" + id);
                $('#modal-edit-category #name_category').val(data.name);
                $('#modal-edit-category #description_category').val(data.description);
            },
            error: function(){
                alert("Nothing Data");
            }
