...
@section('content')
@section('title', 'LSUP Certificate')
...
<div class="content">
    <a type="button" class="btn btn-sm btn-dark mb-4" href="{{ url('admin/certificates/lsup/add') }}">
                    ...
                    <table id="galleryTable"  class="table table-bordered table-striped table-vcenter js-dataTable-full-pagination">
                        <thead>
                            <tr>
                                ...
                                <th class="fs-xs text-center" style="width: 12%">Agreement Number</th>
                        ...
                        <tbody>
                            @foreach ($certificateLsup as $index => $item)
                            <tr>
                                <td class="fs-xs text-center">{{ ($index+1) }}</td>
                                <td class="fs-sm"></a>
                                    <span class="fs-sm fw-semibold">Bussines Name</span><br>{{ $item->business_name }}<br>
                                    <span class="fs-sm fw-semibold">Business Address</span><br>{{ $item->business_address }}<br>
                                    <span class="fs-sm fw-semibold">Company Name</span><br>{{ $item->company_name }}<br>
                                </td>
                                <td class="fs-sm text-center">{{ $item->agreement_number }}</td>
                                ...
                                <td class="fs-sm text-center">
                                    {{ date('d F Y', strtotime($item->certificate_date))}}
                                </td>
                                <td class="fs-sm text-center">
                                    {{ date('d F Y', strtotime($item->issue_date))}}
                                </td>
                                <td class="fs-sm text-center">
                                ...
                                <td class="fs-sm text-center">
                                    <div class="btn-group">
                                        <a type="button" class="btn btn-sm btn-primary" href="{{ url('admin/certificates/lsup/edit/' . $item->id) }}">
                                            <i class="fa fa-fw fa-pencil-alt"></i>
                                        </a>
                                        <a type="button" data-id="{{ $item->id }}" class="btn btn-sm btn-danger btnDelete">
                                            <i class="fa fa-fw fa-trash-can"></i>
                                        </a>
                                        <a type="button" class="btn btn-sm btn-success" href="{{ url('admin/certificates/lsup/generateCode/'.$item->id) }}">
...
<script type="text/javascript">
    $('#galleryTable').on("click", ".btnDelete", function() {
            ...
            if (result.value) {
                let url = "{{ url('admin/certificates/lsup/delete/:id') }}";
